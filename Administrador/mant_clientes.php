...
if (isset($_POST['send_insert'])) {
  ...
  if ($telefono_fijo == "") {
    $conn->insertar(
      "telefonoparticipante(numeroTelefono,idTelefono,idParticipante)",
  ...
  /* Registro del participante en un modulo */
  $conn->insertar("participantecurso(idParticipante,idCurso)", "$id_participante,$curso");
  /* Fin registro participante Modulo */
  ...
  echo " <script>window.location.replace('./index.php?x=clientes.php')</script>";
...
?>
    ...
    <div id="id01" class="w3-modal">
      <div class="w3-modal-content w3-card-4 w3-animate-zoom" style="max-width:700px">
        <div class="w3-center"><br>
          <a href="index.php?x=clientes.php"> <span class="w3-button w3-xlarge w3-hover-red w3-display-topright" title="Close Modal">&times;</span></a>
        ...
        <form class="w3-container" method="post" action="">
          <div class="w3-section">
            ...
            $busqueda = $conn->busquedaFree("SELECT * FROM participante INNER JOIN `dawproyecto`.`participantecurso`
            ON (
          ...
          ON (
          ...
          ) WHERE participante.idParticipante = $id_update");
            ...
            ?>
              ...
              <input class="w3-input w3-border " type="text" Value="<?php echo $nombre; ?>" name="nombre" required pattern="[A-Za-z]+" title="No se permiten Numeros">

              <label><b>Apellido</b></label>
              <input class="w3-input w3-border" type="text" Value="<?php echo $apellidos; ?>" name="apellido" required pattern="[A-Za-z]+" title="No se permiten Numeros">

              <label><b>Telefono Fijo</b></label>
              <?php
               $busqueda_telefono = $conn->busquedaFree("SELECT * FROM telefonoparticipante WHERE idParticipante = $id");
               foreach ($busqueda_telefono as $datos) {
                $idtelefono = $datos['idTelefono'];
                 $numeroTelefono = $datos['numeroTelefono'];
              if ($idtelefono == '1') {
                echo '<input class="w3-input w3-border" type="text" value="'.$numeroTelefono.'" name="telefono_personal" id="telp" required pattern="[0-9]+" title="No se permiten Letras" maxlength="8">';
              }else{
                echo '<label><b>Telefono Celular</b></label>
                <input class="w3-input w3-border" type="text" value="'.$numeroTelefono.'" name="telefono_fijo" id="telc" required pattern="[0-9]+" title="No se permiten Letras" maxlength="8">';
              }}
              ?>
              ...
              <label><b>DUI</b></label>
              <input class="w3-input w3-border" type="text" Value="<?php echo $dui; ?>" name="dui" required pattern="[0-9\-]+" title="No se permiten Letras" maxlength="10">

              <label><b>NIT</b></label>
              <input class="w3-input w3-border" type="text" Value="<?php echo $nit; ?>" name="nit" required pattern="[0-9\-]+" title="No se permiten Letras" maxlength="17">
        ...
        <div class="w3-container w3-border-top w3-padding-16 w3-light-grey">
          <a href="index.php?x=clientes.php" class="w3-button w3-red">Cancel</a>
        </div>
      </div>
    </div>
    <script>
  ...
  $consulta_pc = $conn->actualizar("participantecurso","idCurso = $curso","idParticipante=$id_update");
